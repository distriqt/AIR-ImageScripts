...
xcodebuild -quiet -project "${SCRIPTDIR}/ios/AIRAssets.xcodeproj/" -scheme AIRAssets build

find "${SCRIPTDIR}" -name .DS_Store -delete
cp -Rf "${SCRIPTDIR}/ios/AIRAssets/Assets.xcassets" "${PACKAGEDIR}/." 
cp -Rf "${SCRIPTDIR}/ios/Build/Products/Release-iphoneos/AIRAssets.app/Base.lproj/LaunchScreen.storyboardc" "${PACKAGEDIR}/."
